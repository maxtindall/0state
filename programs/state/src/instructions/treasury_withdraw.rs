...
use crate::{constants::*, error::StateError, records::Spent};

/// Execute a treasury spend authorized by a passed 0state proposal.
///
/// The treasury is owned by a program PDA with no private key, so STATE can
...
///
/// state decodes the proposal by hand rather than depending on the zerostate
...
pub struct TreasuryWithdraw<'info> {
    ...
    #[account(owner = ZEROSTATE_PROGRAM @ StateError::NotAZerostateProposal)]
...
pub fn handler(ctx: Context<TreasuryWithdraw>) -> Result<()> {
    ...
    require!(data.len() >= 8, StateError::NotAZerostateProposal);
    require!(data[0..8] == PROPOSAL_DISCRIMINATOR, StateError::NotAZerostateProposal);
    ...
    let mut o = 8 + 1 + 8 + 32;
    require!(o + 4 <= data.len(), StateError::NotAZerostateProposal);
    let tlen = u32::from_le_bytes(data[o..o + 4].try_into().unwrap()) as usize;
    o += 4 + tlen + 32 + 8; // title, body_hash, created_ts
    require!(o + 8 + 8 + 8 + 8 + 8 + 32 + 8 <= data.len(), StateError::NotASpendProposal);
    ...
    require!(spend_amount > 0, StateError::NotASpendProposal);
    require!(clock.unix_timestamp >= closes_ts, StateError::ProposalNotPassed); // voting closed
    require!(yes > no, StateError::ProposalNotPassed);
    require!(spend_recipient == ctx.accounts.recipient.key(), StateError::RecipientMismatch);
    require!(ctx.accounts.treasury_ata.amount >= spend_amount, StateError::InsufficientTreasury);
