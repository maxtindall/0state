...
use crate::{constants::*, records::Config};
...
/// path to new STATE is proof-of-work via `mine`. There is no `mint` admin
...
pub fn handler(ctx: Context<Initialize>, difficulty: u8, cooldown: i64) -> Result<()> {
    ...
    cfg.reserved = [0u8; 64];
    msg!("state genesis: uncapped (tail {} base units/proof), difficulty {}, cooldown {}s",
        TAIL_REWARD, difficulty, cooldown);
    Ok(())
}
