...
use crate::{constants::*, error::StateError, records::Config};

/// Attach a Metaplex Token Metadata account to the mint so wallets and explorers
/// show "state" instead of a generic SPL token.
///
...
/// own mint authority.
///
/// It grants no new standing power: it is gated to the program's **upgrade
/// authority** (the trusted, pre-mainnet role that is revoked at launch), so no
/// one can front-run it to seize the token's identity. The metadata is created
/// mutable with its update authority handed to a normal wallet, so name/symbol/
/// logo can be refined with standard Metaplex tooling and then frozen
/// (`is_mutable = false`) at mainnet, alongside revoking the upgrade authority.
#[derive(Accounts)]
pub struct CreateMetadata<'info> {
    ...
    pub program: Program<'info, crate::program::State>,
    #[account(
        constraint = program_data.upgrade_authority_address == Some(payer.key())
            @ StateError::NotUpgradeAuthority
    )]
...
pub fn handler(ctx: Context<CreateMetadata>, name: String, symbol: String, uri: String) -> Result<()> {
    ...
    create_metadata_accounts_v3(
        ...
        data,
        true,  // is_mutable — refine now, freeze at mainnet
