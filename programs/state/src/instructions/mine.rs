...
use crate::{constants::*, error::StateError, records::{Config, Proof}};
...
pub struct Mine<'info> {
    ...
    /// STATE routed here can only leave via a governance-gated instruction —
...
pub fn handler(ctx: Context<Mine>, nonce: u64) -> Result<()> {
    ...
    //    so emission continues at a vanishing (asymptotically-zero) inflation
    //    rate rather than stopping at a wall.
    require!(
    ...
    require!(
        ...
        StateError::InsufficientDifficulty
    );

    // 3. Reward for the current point on the emission curve. Never zero — past
    //    the distribution phase it is exactly TAIL_REWARD.
    ...
    cfg.total_minted = cfg.total_minted.checked_add(reward).ok_or(StateError::Overflow)?;
    cfg.proofs_accepted = cfg.proofs_accepted.saturating_add(1);

    // 6a. Difficulty retarget. Once a full window of proofs has been accepted,
    ...
    //     is off by more than 2×. This keeps issuance time-paced (not merely
    //     hashpower-paced) and lifts the production-cost floor as miners join.
    //     Never below the genesis floor, never above the ceiling. The `> 0`
    //     guard makes a not-yet-migrated Config (zeroed retarget fields) inert.
    ...
    proof.total_mined = proof.total_mined.checked_add(reward).ok_or(StateError::Overflow)?;
...
/// The emission curve. Starts at INITIAL_REWARD (500 STATE) and halves once per
/// supply tranche across the distribution phase — tranche 0 spans the first 500M
/// STATE at 500/proof, tranche 1 the next 250M at 250/proof, and so on — then
/// **floors at TAIL_REWARD and stays there forever**. It never returns zero, so
/// mining is uncapped: past the distribution phase every proof mints the tail,
/// a fixed absolute amount whose share of an ever-growing supply tends to zero.
...
mod reward_tests {
    ...
    fn reward_halves_each_tranche() {
        // tranche 1 starts at 500M cumulative -> reward 250 STATE
        assert_eq!(reward_for(DISTRIBUTION_PHASE / 2), INITIAL_REWARD / 2);
        // tranche 2 starts at 750M cumulative -> reward 125 STATE
    ...
    fn emission_is_uncapped_and_floors_at_tail() {
        // Far past the distribution phase the reward is exactly the tail and
        // never zero, so mining continues forever.
    ...
    fn reward_is_monotonic_down_to_the_tail() {
        // Walk the curve outward: it only ever decreases, never below the tail,
        // and settles exactly at the tail.
