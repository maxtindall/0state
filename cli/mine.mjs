...
const PROGRAM_ID = new PublicKey(process.env.STATE_PROGRAM || '2xvfGKpTHy5EA8RFJsKXu73nJVFPRH6YHxDAE5Rh6SVE');
...
// The commons treasury: a keyless program PDA and its token account. 10% of
// every reward is minted here automatically; the miner must pass both accounts.
...
async function main() {
  ...
  let idl = await anchor.Program.fetchIdl(PROGRAM_ID, provider).catch(() => null);
  if (!idl) {
    const here = path.dirname(fileURLToPath(import.meta.url));
    const local = [
      path.join(here, 'state.idl.json'),
      path.join(here, '..', '..', 'openbot', 'chain', 'frankcoin', 'target', 'idl', 'state.json'),
    ].find(p => fs.existsSync(p));
    if (!local) throw new Error('no on-chain IDL and no bundled IDL found (state.idl.json)');
    idl = JSON.parse(fs.readFileSync(local));
    console.log('  IDL      (local)', local);
  } else {
